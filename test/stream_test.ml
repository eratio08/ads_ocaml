...
let stream = Alcotest.testable StreamTest.pp StreamTest.equal
...
module StreamFn_test = struct
  let rec pp_fn fmt = function
    | StreamFn.Empty -> Format.fprintf fmt "Empty"
    | StreamFn.Cons { head; tail } ->
      Format.fprintf
        fmt
...
let stream_fn = Alcotest.testable StreamFn_test.pp_fn StreamFn_test.equal
...
let suite =
  ...
  ; "StreamFn.(++)", `Quick, test_plusplus_fn
  ; "StreamFn.take", `Quick, test_take_fn
  ; "StreamFn.drop", `Quick, test_drop_fn
  ; "StreamFn.reverse", `Quick, test_reverse_fn
  ]
;;
